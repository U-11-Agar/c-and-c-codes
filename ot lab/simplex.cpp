...
int n;
int m; // m=no of constrant
int z;
int index_;
vector<vector<double> > x(n, vector<double>(m));
vector<double> x_z(n + m);           // z can be writen as in the + non basic variable and slack varible
vector<vector<double> > slack(m, vector<double>(m)); // slack variable
vector<double> c(m);             // constant
vector<double> sol(m+n);
double z_con;
...
void optimize_value(vector<double> sol,vector<double> x_z,double z_con)
{
    double bfs = 0;
    for (int i = 0; i < n + m; i++)
    {
...
void update_x_z(vector<double> x_z,vector<vector<double> > slack,vector<vector<double> > x,vector<double> sol,double z_con)
{
    double demo;
    demo = x_z[z];
    x_z[z] = 0;
...
void update_x(vector<double> x_z,vector<vector<double> > slack,vector<vector<double> > x,vector<double> sol,double z_con,vector<double> c,int index_)
{
    ...
    cout << ")\n";
    update_x_z(x_z,slack,x,sol,z_con);
    return;
}
void loop(vector<double> q, vector<double> c) // this function will campare the division of constant and to the non basic varialbe
{
    double min = 0;
    double demo;
...
int main(void)
{
    ...
    for (int i = 0; i < n; i++)
    {
        cout << "input the coffi of basic variable in the constraints equation the in order all the x1 then x2 and  so on" << endl;
        for (int j = 0; j < m; j++)
        {
            printf("input the x%d in the %d constant eq\n", i, j);
            cin >> x[i][j];
        }
    }
    cout << "what do you want to do with the obective function i.e. u want to max or min input 1 if Min other wise 0" << endl;
    int demo;
    cin >> demo;
    if (demo != 1)
    {
